...
\begin{itemize}
    ...
    \item \textbf{\texttt{threading}}: para a execução de tarefas em \textit{threads} separadas,
    permitindo a execução de tarefas em paralelo;
    \item \textbf{\texttt{json}}: para a codificação e descodificação de mensagens em formato JSON,
    permitindo a transmissão de dados entre o servidor e os agentes;
    \item \textbf{\texttt{os}}: para a execução de comandos do sistema operativo, permitindo a
    recolha de métricas de desempenho dos dispositivos;
    \item \textbf{\texttt{sys}}: para a obtenção de argumentos passados na linha de comandos,
    ...
    \item \textbf{\texttt{iperf3}}: para a execução de testes de largura de banda e \textit{jitter},
    permitindo a obtenção de informações sobre esses dados entre sistemas.
    \item \textbf{\texttt{subprocess}}: utilizado para correr o comando nativo \textit{ping}, obtendo
...
\begin{itemize}
    ...
    Definiu-se como sendo \textbf{5 segundos}, permitindo uma rápida recuperação de pacotes perdidos sem causar sobrecarga excessiva na rede.
    ...
    \item \textbf{\texttt{ENCODING}}: a codificação de mensagens foi pré-definida como \textbf{\textit{UTF-8}}, podendo esta ser configurável, 
    como, por exemplo, \textit{ASCII}, \textit{ISO-8859-1}, entre outras;
    \item \textbf{\texttt{BUFFER\_SIZE}}: o tamanho máximo de um pacote foi definido como sendo \textbf{1500 \textit{bytes}},
    que é o valor máximo para o \textit{Maximum Transmission Unit} (MTU) de uma rede Ethernet;
    \item \textbf{\texttt{SO\_NO\_CHECK}}: definido como \textbf{11}, este valor específico desativa o cálculo do \textit{checksum} em
    pacotes UDP, permitindo a aplicação de um controlo de erros personalizado;
    \item \textbf{\texttt{WINDOW\_PROBE\_SLEEP\_TIME}}: intervalo de tempo, em segundos, antes de verificar o \textit{window size}.
    Definiu-se como sendo \textbf{10 segundos}, permitindo uma verificação regular da janela de receção sem causar sobrecarga excessiva na rede.
...
\begin{minipage}{0.49\textwidth}
    ...
    \captionof{figure}{Gráfico de \textit{Bandwidth}}
    \label{fig:bandwidth}
\end{minipage}
\hfill
\begin{minipage}{0.49\textwidth}
    ...
    \captionof{figure}{Gráfico de \textit{Packet Loss}}
...
\begin{minipage}{0.49\textwidth}
    ...
    \captionof{figure}{Gráfico de \textit{Jitter}}
    \label{fig:jitter}
\end{minipage}
\hfill
\begin{minipage}{0.49\textwidth}
    ...
    \captionof{figure}{Gráfico de Latência}
    \label{fig:latency}
\end{minipage}
...
\begin{minipage}{0.49\textwidth}
    ...
    \captionof{figure}{Gráfico de Utilização da CPU}
    \label{fig:cpu_usage}
\end{minipage}
\hfill
\begin{minipage}{0.49\textwidth}
    ...
    \captionof{figure}{Gráfico de Utilização de RAM}
    \label{fig:ram_usage}
\end{minipage}
