...
\begin{itemize}
    ...
    \item \textbf{\textit{threading}}: para a execução \textit{multi threading} de tarefas, servidores TCP e UDP,
...
\item \textbf{\textit{iperf3}}: \textit{iperf wrapper} para a execução de testes de largura de banda, \textit{jitter} e \textit{packet loss},
    permitindo a obtenção facilitada desses dados aquando o \textit{iperf} é executado como cliente;
\item \textbf{\textit{subprocess}}: utilizado para correr os comando nativos \textit{ping} e \textit{iperf3} (modo servidor),
...
\begin{itemize}
    ...
    A definição deste valor teve em conta que a retransmissão de pacotes precisa de ter um tempo de resposta curto para garantir
    o desempenho, porém, não tão curto como, por exemplo, nos serviços de \textit{streaming}, onde a latência é crítica.
    Manter este valor significamente maior reduz a sobrecarga computacional e de tráfego de rede.
    \item \textbf{\texttt{INITIAL\_WINDOW\_SIZE}}: o \textit{window size} foi definido como sendo \textbf{32 pacotes}.
    Este valor é suficientemente para lidar com tráfego usual da aplicação, sem causar congestionamento na rede.
